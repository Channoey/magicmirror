...
class ThirdPartInfo:
    ...
    def weather(self):
        ...
        url = r'https://tianqi.2345.com/hangzhou1d/58457.htm'
        ...
        humid = soup.find_all('span', class_='real-data-mess')[1].get_text().strip()
        humid = humid[-3] + humid[-2] + humid[-1]
        print("humid:" + humid)
        return [weather, imgiconURL, temperature, wind_force, emoticon, humid]
...
class ExQThread(QThread):
    ...
    def updateTempHum(self):
        ...
        # print('>> locate:', locate)
        res = self.thirdPart.weather()
        self.tempHumSignal.emit(res[2], res[-1])
    ...
    # self.tempHumSignal.emit("test1", "test2")
    ...
    def updateWeather(self):
        ...
        weather = res[0]
        iconURL = res[1]
        path = "/home/magicmirror/Desktop/pycharm_project_214/PythonProject/source/icon/" + weather + ".png"
...
class SpImgThread(QThread):
    ...
    def screenWakeUp(self):
        os.popen('sudo vcgencmd display_power 1')  # 唤醒

    # 检查屏幕的点亮状态
    def checkScreenStatOn(self):
        try:
            stat = os.popen('sudo vcgencmd display_power').readlines()[-1].strip()[-1]
        except:
            return True
        if stat == '0':
            return False
    ...
    def setScreenOff(self):
        os.popen('sudo vcgencmd display_power 0')  # 立即关闭屏幕
    ...
    # 人脸对比
    def compareFace(self, image_path="/home/magicmirror/Desktop/pycharm_project_214/PythonProject/facecompare",
    ...
    def stateCheck(self):
        ...
        if self.MODE == 0:  # 等待状态
            ...
            print(">> 有人靠近状态")
            res = self.isSomeoneClose()
            if res == 1:
                self.MODE = 4
            if self.isnihao:
                self.speech.playAudio("/home/magicmirror/Desktop/pycharm_project_214/PythonProject/audio/nihao.mp3")
                while self.speech.checkAudioPlaying():  # 检查音频播放线程是否结束
                    time.sleep(1)
                self.isnihao = False
            # msg = self.onePhrase()
            # self.speech.TextToPlay(msg)
            if self.speech.isface:
                self.speech.TextToPlay("只有徐茂桓才可以使用这个功能哦～")
                while self.speech.checkAudioPlaying():  # 检查音频播放线程是否结束
                    time.sleep(1)
                compareScore = self.compareFace()
                print("comparescore:" + str(compareScore))
                if compareScore == 1000:
                    self.speech.TextToPlay("你是金城武，很帅，但是也不能使用人脸检测功能哦～")
                    while self.speech.checkAudioPlaying():  # 检查音频播放线程是否结束
                        time.sleep(1)
                    self.speech.isface = False
                elif compareScore == 91:
                    self.MODE = 2  # 改变状态
                    self.speech.TextToPlay(
                        "你是徐茂桓，比金城武还帅一点，开始人脸检测功能，检测到微笑会自动停止～")
                    while self.speech.checkAudioPlaying():  # 检查音频播放线程是否结束
                        time.sleep(1)
                else:
                    self.speech.isface = False
                    self.speech.TextToPlay("我不认识你，已关闭人脸检测程序～")
                    while self.speech.checkAudioPlaying():  # 检查音频播放线程是否结束
                        time.sleep(1)
            ...
            if self.face.getHappyFace == 1:  # 若开心
                print(">> 检测到开心，停止人脸检测")
                self.communicateSignal.emit("检测到开心，停止人脸检测！！")
                self.speech.textSpeechFunc("好开心")
                while self.speech.checkAudioPlaying():  # 检查音频播放线程是否结束
                    time.sleep(1)
                # self.speech.TextToPlay("哇，你好开心啊，我已经停止检测了！")
                # self.setLEDoff()
                ...
                print(">> 检测到不开心")
                self.communicateSignal.emit("检测到不开心！！")
                self.speech.textSpeechFunc("好伤心，快安慰我")
                # self.speech.TextToPlay("你为什么不开心呀？") #TODO1
                # self.speech.playAudio("/home/magicmirror/Desktop/pycharm_project_214/PythonProject/audio/nihao.mp3")
                while self.speech.checkAudioPlaying():  # 检查音频播放线程是否结束
                    time.sleep(1)
            ...
            if res == 1:  # 没有人了
                print(">> 没有人了3")
                self.speech.playAudio("/home/magicmirror/Desktop/pycharm_project_214/PythonProject/audio/baibai.mp3")
                while self.speech.checkAudioPlaying():  # 检查音频播放线程是否结束
                    time.sleep(1)
    ...
    def run(self):
        ...
        os.close(sys.stderr.fileno())  # 减少警告
...
class MagicUI(Ui_MainWindow, QObject):
    ...
    def setupUi(self, MainWindow):
        ...
        file = "/home/magicmirror/Desktop/pycharm_project_214/PythonProject/img/xiaomei"
        i = random.randint(1,2)
        file = file + str(i) + ".gif"
        self.gif = QMovie(file)
